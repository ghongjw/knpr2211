...
function sendData(sel1, sel2){
    ...
    var id = sel1 + sel2;
    var codeName1 = "B" + sel1 + sel2;
    var codeName2 = "B" + sel1;
    var campsiteCode = "";
    var camp_tag = "";
	var arrayRoom = [];
	var arrayDate = [];
	var shelter_tag= "";
		...
		$.ajax({
			...
			success : function(result) {
				...
				var shelterTotal = list2.length;			

				for(var i = 0; i < list1.length; i++){ 
					
					//2-1. 산이름 출력 (지리산)
					document.querySelector('#category2').innerHTML = "<i class=icon-location></i>" +list1[i].category2;
					
					//2-2. 대피소 출력 (벽소령)
					document.querySelector('#category3').innerHTML = list1[i].category3 +" "+"대피소 예약현황";
				...
				//2-3. 대피소명 출력 (테이블)
					for(var i = 0; i < list2.length; i++){
  						if(i == 0 ){
						 camp_tag +="<tr><th rowspan=15 scope=row><span class=title>대피소</span></th>" +
		            	"<th scope=row><span class=title>" + list2[i].category3 + "</span></th></tr>"
		            	arrayRoom.push(list3[i].category3)
		      
 	
					}else{
	 			
	 					camp_tag += "<tr><th scope=row><span class=title>" + list2[i].category3 + "</span></th></tr>"
	 					arrayRoom.push(list3[i].category3)
				...
				document.querySelector('.month').innerHTML = month_tag;  
	 			...
	 			for(var i = 0; i < 23; i++){
	 				if(dateCheck >= i){
	 					...
	 					if(inputDay == "sat"){
	 						shelter_tag += "<td id='sat' class='sat'>"+shelterTotal+"</td>"
 						
	 					}else if(inputDay == "sun"){
	 						shelter_tag += "<td id='sun' class='sun'>"+shelterTotal+"</td>"
 						
	 					}else{
	 						shelter_tag += "<td>"+shelterTotal+"</td>"
	 					}
	 			...
	 			STreservationState(arrayRoom, arrayDate)


	 			document.querySelector('#campsiteName').innerHTML = camp_tag; 
	 			document.querySelector('.roomTotal').innerHTML = shelter_tag;
 ...
 //6. 현재 예약 상태 조회 후 예약가능/불가능 반환
  function STreservationState(arrayRoom, arrayDate){
   var state_tag = "";
   var roomMax_tag = "";
   var num = 1;
   var arrayState = [];
 ...
 function oneNightCheck(room, date, state){
	 ...
	 $.ajax({
			...
			success : function(result) {
				var dateList = result.dateList;
				var nextRoom = dateList.room1
				var codeRoom = dateList.room2
				var nextDate = dateList.date
				...
				if(nextState == "예약가능"){
					...
					for(var i = 0; i < tds.length; i++){
 					...
 					document.getElementById("a"+room+date).className = 'start selected';
  					...
  					$('#category3').html(codeRoom+" 대피소 예약현황")
  					//document.querySelector('#category3').innerHTML = 
  					//console.log(codeRoom)
  					STinputSelectInfo(room, date, state)
									
					
				}else {
					toastrMsg("선택한 날짜는 예약이 불가합니다. 다른 날짜를 선택해 주세요."); 			
				}
...
// 8. 사용자가 선택했을 때, 예약이 가능하면 장소 정보 화면에 출력
function STinputSelectInfo(room, date, state){
	
	 $.ajax({
			url : "STinputSelectInfo",
			type : "post",
			cache : false,
			data : {
				room : room,
			...
			success : function(result) {
				...
				$('#stayInfo').html("1일")
					...
					var totalMoney = money.toLocaleString(); 

	
	
                var pay_tag = "<dl><dt><em>"+c1+"</em></dt><dd></dd></dl>"+
                "<dl class='campsitePayment'><dt>"+c2+" "+c3+"대피소 : "+
                "<span style='font-size: 18px; font-weight:bold;' id='inputAllDay'>1일"+
	...
	<div class="page-location">
     ...
     <div class="reservation" data-device-mode="pc">
         ...
         <div class="nodata" id="nodata1">
             ...
             <span>선택된 대피소가 없습니다. 대피소를 선택해주세요.</span>
         </div>
         ...
         <div class="content-view" id="content-view" style="display:none;">
                    <div class="tab-pane is-active" id="tab14-5">
                        <div class="title-area">
                            ...
                            <h4 class="title" id="category3">대피소 예약현황</h4><!-- 야영장 예약현황 -->
...
<!--                                 <button class="btn btn-360" onclick="javascript:imgView('/product/camp/camp0602_2_210611.jpg');">
                       ...
                       <script> 
                        ...
                        <div class="table-row">
                            <div class="item">
                                <em class="label"><i class="icon-camp"></i>대피소</em>
                            ...
                            <div class="item">
                                ...
                                <div class="stay-row" id="stayInfo" style="font-size:20px;
                                align-items: center; min-height: 40px; margin-top: 20px;">1일</div>
                            </div>
                        </div>
               ...
               <div class="popup-wrap" style="top: 65px; bottom: 150px;">
                  ...
                  <div class="popup-container">
                        ...
                        <div class="captcha-area" style="margin: 0;">
                           ...
                           <span class="g-recaptcha" data-sitekey="6LfhDXAjAAAAAFFHjdQyjtpVUaoehFWEksSN372O"></span>
                        </div>
